...
//import { AddToCartButtonStyled } from './styles'
...
function AddToCartButton(props: Props) {
  ...
  const resolveToastMessage = (success: boolean) => {
    if (!success) return 'Error al agregar al carrito'
    const { quantity } = skuItems[0]
    return `Se agregaron ${quantity}un. al carrito`
  }
  ...
  const handleAddToCart: React.MouseEventHandler = (event) => {
    ...
    addToCartFeedback === 'customEvent' &&
      (timers.current.toast = window.setTimeout(() => {
        customFeedbackEvent(resolveToastMessage(true))
      }, FAKE_LOADING_DURATION))
  }
  ...
  const availableButtonContent = (
    <div className={`${handles.buttonDataContainer} flex justify-center`}>
      <img
        src="https://arvitalqa.vteximg.com.br/arquivos/Cart.svg"
        alt={text}
      />
    </div>
  )
